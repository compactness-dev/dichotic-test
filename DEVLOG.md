...
The first challenge was getting speech audio into a self-contained HTML file. The standard Dichotic Digits Test uses spoken digits (1–9), and `espeak` was available on the system. WAV files were generated, base64-encoded, and embedded directly in the HTML. No external files, no server — just open it in a browser.
...
An unexpected UX problem emerged: when the response grid order was randomized (left grid vs right grid shown first), it was easy to lose track of which ear heard what, producing noisy data. A **Fixed/Randomized toggle** was added so users can choose their preferred response order.
...
A diagnostic panel was built to synthesize all session data:
...
The UI was redesigned with a light theme, refined typography (DM Sans + JetBrains Mono), and sample results were added so new users can see what the output looks like before running a test.
...
4. **Iterative development.** Every feature came from running the test and noticing something: too easy → add syllables. Bad audio → switch TTS. Confusing grid order → add toggle. The conversation was the development process.
